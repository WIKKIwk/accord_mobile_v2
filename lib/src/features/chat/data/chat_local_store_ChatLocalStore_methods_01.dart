...
extension ChatLocalStoreAstPart01 on ChatLocalStore {
  Future<Database> _open() async {
    ...
    final database = await openDatabase(
      ...
      onCreate: (db, _) async {
        ...
        ''');
        await ChatLocalStore._createPendingTable(db);
        await ChatLocalStore._createPendingMediaTable(db);
        await ChatLocalStore._createSyncStateTable(db);
        await ChatLocalStore._createReceiptTable(db);
      },
      onUpgrade: (db, oldVersion, _) async {
        if (oldVersion < 2) await ChatLocalStore._createPendingTable(db);
        if (oldVersion < 3) await ChatLocalStore._createPendingMediaTable(db);
        ...
        if (oldVersion < 4) await ChatLocalStore._createSyncStateTable(db);
        if (oldVersion < 5) await ChatLocalStore._createReceiptTable(db);
  ...
  Future<List<ChatConversation>> loadConversations(String profileKey) async {
    ...
    final rows = await db.query(
      ...
      limit: ChatLocalStore._initialCacheLimit,
    );
    return rows
